...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(self, state, encoder_padding_mask):
        ...
        # query.size = [src_time_steps, batch_size, num_features]
        ...
        # _.size = [num_heads, batch_size, tgt_time_steps, src_time_steps]
        '''
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self,
        ...
        3.  What is the difference between key_padding_mask and attn_mask?   
            # key_padding_mask: mask the padding, attn_mask: mask 预测中自己跟自己的attention
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self,
        ...
        # Key = self.k_proj(key)
        #
        # # [batch*num, tgt/src , head_embed_size]
        # Query = Query.contiguous().view(-1, batch_size * self.num_heads, self.head_embed_size).transpose(0, 1)
        # Value = Value.contiguous().view(-1, batch_size * self.num_heads, self.head_embed_size).transpose(0, 1)
        # Key = Key.contiguous().view(-1, batch_size * self.num_heads, self.head_embed_size).transpose(0, 1)
        #
        # # Query = Query.contiguous().view(batch_size * self.num_heads, -1, self.head_embed_size)
        # # Value = Value.contiguous().view(batch_size * self.num_heads, - 1, self.head_embed_size)
        # # Key = Key.contiguous().view(batch_size * self.num_heads, - 1, self.head_embed_size)
        #
        # # [batch_size*num_heads, tgt_time_steps, src_time_steps]
        # scores = torch.matmul(Query, Key.transpose(-1, -2)) / self.head_scaling
        #
        # if key_padding_mask is not None: # [batch_size, tgt_time_steps]
        #     # [batch_size, 1, tgt_time_steps]
        #     key_padding_mask = key_padding_mask.unsqueeze(1).repeat(self.num_heads, 1, 1)
        #     scores.masked_fill(key_padding_mask, float('-inf'))
        # if attn_mask is not None:
        #     attn_mask = attn_mask.unsqueeze(0)
        #     scores += attn_mask
        # scores = torch.softmax(scores, dim=-1)
        #
        # # [batch_size * num_heads, tgt_time_steps, head_embed_size]
        ...
        #
        # # attn_weights must be size [num_heads, batch_size, tgt_time_steps, key.size(0)]
        # attn_weights = scores.contiguous().view(self.num_heads, batch_size, tgt_time_steps, -1)
        ...
        if attn_mask is not None:  # [seq_len, seq_len]
            ...
            # assert tgt_time_steps == key.size()[0]
