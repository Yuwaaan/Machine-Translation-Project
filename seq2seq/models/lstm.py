...
class LSTMEncoder(Seq2SeqEncoder):
    ...
    def forward(self, src_tokens, src_lengths):
        ...
        # state_size.size = [2*num_layers, batch_size, hidden_size]

        hidden_initial = src_embeddings.new_zeros(*state_size)  # [2*num_layers, batch_size, hidden_size]
        context_initial = src_embeddings.new_zeros(*state_size)  # [2*num_layers, batch_size, hidden_size]
        ...
        if self.bidirectional:
            ...
            final_hidden_states = combine_directions(final_hidden_states) # [2, 10, 64] -> [1, 10, 128]
            # final_hidden_states.size =  [num_layers, batch_size, 2*hidden_size]
            final_cell_states = combine_directions(final_cell_states) # [2, 10, 64] -> [1, 10, 128]
...
class AttentionLayer(nn.Module):
    ...
    def forward(self, tgt_input, encoder_out, src_mask):
        ...
        attn_scores = self.score(tgt_input, encoder_out)  # [10, 1, 12]
        ...
        if src_mask is not None:
            ...
            # scr_mask.size = [batch_size, 1, src_time_steps]
            attn_scores.masked_fill_(src_mask, float('-inf'))
            # attn_scores.size = [batch_size, 1, src_time_steps]

        attn_weights = F.softmax(attn_scores, dim=-1)  # [10, 1, 12]
        ...
        # tgt_input: [10, 128], attn_context: [10, 128] --> context_plus_hidden: [10, 256]
        ...
        # in tanh: [10, 128], attn_out: [10, 128]
    ...
    def score(self, tgt_input, encoder_out):
        ...
        '''
        # (h_t)tgt_input.size = [batch_size, input_dims]
        # (h_s)encoder_out.size = [batch_size, src_time_steps, output_dims]
        ...
        # mat1:[batch_size, 1, input_dims], mat2:[batch_size, output_dims, src_time_steps]
        # Out:[batch_size, 1, src_time_steps]
        # attn_scores.size = [batch_size, 1, src_time_steps]
...
class LSTMDecoder(Seq2SeqDecoder):
    ...
    def forward(self, tgt_inputs, encoder_out, incremental_state=None):
        ...
        tgt_embeddings = tgt_embeddings.transpose(0, 1)  # [23, 10, 64]
        # tgt_embeddings.size = [tgt_time_steps, batch_size, num_features]
        ...
        # cached_state.size = [num_layers, batch_size, hidden_size]

        if cached_state is not None:
            tgt_hidden_states, tgt_cell_states, input_feed = cached_state
        else:
            tgt_hidden_states = [torch.zeros(tgt_inputs.size()[0], self.hidden_size) for i in range(len(self.layers))]  # list
            # tgt_inputs.size = [batch_size, tgt_time_steps]
            ...
            # tgt_embeddings.size = [tgt_time_steps, batch_size, num_features] [x, 10, 64]
            # input_feed.size = [batch_size, hidden_size]  # [10, 128]
        ...
        for j in range(tgt_time_steps):
            ...
            if self.attention is None:
                ...
                # attn_weights = [batch_size, tgt_time_step, src_time_step]
                if self.use_lexical_model:
                    # __QUESTION-5: Compute and collect LEXICAL MODEL context vectors here
                    # TODO: --------------------------------------------------------------------- CUT
                    # torch.unsqueeze(step_attn_weights,1)  # [batch_size, src_time_steps] --> [batch_size, 1, src_time_steps]
                    # src_embeddings.transpose(0, 1) # [src_time_steps,batch_size,embed_dim] --> [batch_size, src_time_steps,embed_dim]
                    f_t = torch.tanh(torch.bmm(step_attn_weights.unsqueeze(dim=1), src_embeddings.transpose(0, 1))).squeeze(1)
                    # [batch_size, 1,embed_dim] --> [batch_size ,embed_dim]
                    h_t = torch.tanh(self.lexical_hidden(f_t)) + f_t
        ...
        if self.use_lexical_model:
            ...
            # TODO: --------------------------------------------------------------------- CUT
            lex_out = torch.stack(lexical_contexts).transpose(0, 1)
